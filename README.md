...
| | FP16 | **W8A8 (наша)** | Δ |
|---|---|---|---|
| Хранение на диске | 15.26 ГиБ | **8.79 ГиБ** | **−42%** |
| Память GPU (веса) | 15.27 ГиБ | **8.80 ГиБ** | **−42%** |
| Скорость (vLLM, H200) | 78.2 tok/s | **114.0 tok/s** | **+46%** |
| Качество: HellaSwag (полный), acc_norm | 0.750 ± .004 | 0.754 ± .004 | **паритет** |
| Качество: GSM8K (5-shot), exact-match | 0.906 ± .013 | 0.910 ± .013 | **паритет** |

Качество проверено на двух независимых бенчмарках (lm-eval-harness, один
seed и путь для обеих моделей): различия в пределах статистической
погрешности — сжатие вдвое без измеримой потери качества. Perplexity и полная
методика — в [REPORT.md](REPORT.md).
