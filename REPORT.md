...
Качество дополнительно проверено на двух независимых бенчмарках через
lm-eval-harness (одинаковые seed, токенизатор и путь через API для обеих
моделей):

| Бенчмарк | FP16 | W8A8 | Вердикт |
|---|---|---|---|
| HellaSwag (полный, 10042, 0-shot), acc_norm | 0.7500 ± 0.0043 | 0.7542 ± 0.0043 | идентично в пределах погрешности |
| GSM8K (500, 5-shot), strict exact-match | 0.906 ± 0.013 | 0.910 ± 0.013 | идентично в пределах погрешности |

На всех замерах (perplexity, HellaSwag, GSM8K) различия между FP16 и W8A8
лежат внутри статистической погрешности: W8A8 для модели такого размера —
мягкий режим, а GPTQ компенсирует ошибку округления подгонкой оставшихся
весов. Вывод: сжатие вдвое без измеримой потери качества.
